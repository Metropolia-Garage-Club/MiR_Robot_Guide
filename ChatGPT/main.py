...
class Chatbot:
    ...
    LOCATIONS = {"ulyseus": 0, "vessa": 1, "wc": 1, "auditorio": 2, "kahvila": 3, "hissi": 4, "kirjasto": 4, "ruokala": 5, "ulyseus toimisto":6, "lataamaan": 7, "takaisin kotiin": 8  }
    ROUTE_TRIGGERS = ["missä", "miten", "vie", "näytä", "johdata", "navigoi", "reitti", "mennään", "opasta", "mene"]
        ...
        try:
            if any(keyword in words for keyword in self.ROUTE_TRIGGERS):
                ...
                if location:
                    ...
                    smalltalk = response = self.chatgpt_instance.gpt_query(str("kerro fakta paikasta: "+ str(location)))
                    self.say(smalltalk)
